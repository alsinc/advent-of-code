...
func read(memory []int, parameterMode int, parameter int) int {
	if parameterMode == 0 {
...
func run(memory []int) []int {
	...
	for program[ip] != 99 {
		...
		switch opcode {
		case 1: //add
			res := read(program, parameterModes[0], program[ip + 1])
			res = res + read(program, parameterModes[1], program[ip + 2])
		...
		case 3: //read int
			...
			ip = ip + 2
		case 4: //print int
			val := read(program, parameterModes[0], program[ip + 1])
			fmt.Println(val)
			ip = ip + 2
		case 5: //jump-if-true
			val := read(program, parameterModes[0], program[ip + 1])
			if val != 0 {
				ip = read(program, parameterModes[1], program[ip + 2])
			} else {
				ip = ip + 3
			}
		...
		case 7: //less than
			val1 := read(program, parameterModes[0], program[ip + 1])
			val2 := read(program, parameterModes[1], program[ip + 2])
			if val1 < val2 {
